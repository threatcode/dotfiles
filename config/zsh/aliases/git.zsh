...
# Shorthand clone (e.g. $ clone threatcode/dotfiles)
function clone {
  default_service='github.com' # Used if full URL isn't specified
  default_username='threatcode' # Used if repo org / username isn't specified
  ...
  if [[ $user_input == --help ]] || [[ $user_input == -h ]]; then
    ...
    echo -e 'E.g. `$ clone threatcode/dotfiles`'
    return;
